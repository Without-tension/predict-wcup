...
def main():
    print("🔍 Запуск розвідки ліг на сервері API-Sports для 2026 року...")
    
    # Запитуємо у API список усіх ліг на 2026 рік
    url = "https://v3.football.api-sports.io/leagues?season=2026"
    response = requests.get(url, headers=HEADERS).json()
    leagues = response.get("response", [])
    
    print(f"Загалом сервер повернув {len(leagues)} ліг для сезону 2026.")
    print("Фільтруємо турніри, які містять 'World' або 'Cup' у назві:\n")
    ...
    for item in leagues:
        ...
        # Шукаємо ключові слова у назві ліги чи країни
        if "world" in league_name.lower() or "cup" in league_name.lower() or "world" in country.lower():
            print(f"🏆 ID: {league_id} | Назва: {league_name} | Тип: {league_type} | Країна: {country}")
            found = True
            
    if not found:
        print("❌ На жаль, жодної ліги за ключовими словами не знайдено. Спробуємо вивести перші 20 ліг для тесту:")
        for item in leagues[:20]:
            league = item.get("league", {})
            print(f"• ID: {league.get('id')} | Назва: {league.get('name')} | Країна: {item.get('country', {}).get('name')}")
