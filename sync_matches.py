...
def fetch_world_cup_matches():
    # league=1 (World Cup), міняємо на season=2025, бо в API всі матчі ЧС зараз там!
    url = "https://v3.football.api-sports.io/fixtures?league=1&season=2025"
    response = requests.get(url, headers=HEADERS).json()
    return response.get("response", [])
...
def main():
    print("🔄 Запуск синхронізації матчів з API-Sports (Сезон 2025)...")
    fixtures = fetch_world_cup_matches()
    print(f"Знайдено {len(fixtures)} матчів у базі провайдера.")

    # Беремо для тесту перші 50 матчів, щоб не перевантажувати базу за один раз
